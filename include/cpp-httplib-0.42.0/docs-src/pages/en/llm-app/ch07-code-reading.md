...
```text
POST /translate          to {"translation": "..."}
POST /translate/stream   to SSE: data: "token"
```

**llama-server's API:**

```text
POST /v1/chat/completions  to OpenAI-compatible JSON
POST /v1/completions       to OpenAI-compatible JSON
POST /v1/embeddings        to Text embedding vectors
```
