...
- `#include <csignal>` to `#include <thread>`, `<cpp-embedlib-httplib.h>`, `"WebAssets.h"`, `"webview/webview.h"`
- `signal_handler`関数を削除
- `svr.set_mount_point("/", "./public")` to `httplib::mount(svr, Web::FS)`
- `svr.listen("127.0.0.1", 8080)` to `bind_to_any_port` + `listen_after_bind` + WebViewのイベントループ
